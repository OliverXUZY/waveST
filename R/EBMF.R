...
EBMF_rank1 = function(Y, tol = 1e-3, maxIter = 1000) {
  ...
  for (iter in 1:maxIter) {
    ...
    LL = ll_res$pMu
    LL2 = matrix(ll_res$pSigma2 + ll_res$pMu^2, nrow = n)
    ...
    FF = ff_res$pMu
    FF2 = matrix(ff_res$pSigma2 + ff_res$pMu^2, nrow = p)
...
EBMF_sigFct = function(Y, LL, FF, LL2, FF2, k) { # update k-th factor
  ...
  ll_res = ebnm(ll_hat, sqrt(s2_hat))
  lk = ll_res$pMu
  lk2 = matrix(ll_res$pSigma2 + ll_res$pMu^2, nrow = n)
  ...
  ff_res = ebnm(ff_hat, sqrt(s2_hat2))
  fk = ff_res$pMu
  fk2 = matrix(ff_res$pSigma2 + ff_res$pMu^2, nrow = p)
